...
!(function () {
					...
					if (this.root || !o || n) {
						...
						this._observationTargets.forEach(function (n) {
							...
							this._rootContainsTarget(i)
								? (o && !this.root) || (l = e)
								: (l = { top: 0, bottom: 0, left: 0, right: 0, width: 0, height: 0 });
							var f = (n.entry = new r({
						...
						for (var s, h, c, a, f, d, g, m, v = i, _ = p(e), b = !1; !b && _; ) {
							...
							if (
								(w &&
									...
									(m = a - c),
									(v =
										((g = d - f) >= 0 && m >= 0 && { top: c, bottom: a, left: f, right: d, width: g, height: m }) ||
										null)),
								!v)
							)
								break;
	...
	function p(e) {
		...
		return 9 == e.nodeType && e != t
			? i(e)
			: (o && o.assignedSlot && (o = o.assignedSlot.parentNode), o && 11 == o.nodeType && o.host ? o.host : o);
	}
	function d(t) {
			...
			$('video[data-auto-play]').each(function () {
				...
				setTimeout(function () {
					if ($('#' + $video.attr('id')).length) {
						if (
							$('[data-trigger-play-video="#' + $video.attr('id') + '"]').data('trigger-play-video-remove') == 'yes'
						) {
	...
	PluginCarouselLight.prototype = {
		...
		events: function () {
			...
			self.$el.on('change.owl.carousel', function (event) {
				...
				self.$el
					.find('[data-appear-animation]:not(.background-image-wrapper), [data-plugin-animated-letters]')
					.addClass('invisible');
		...
		navigationOffsets: function () {
			...
			if (self.options.navVerticalOffset && self.options.navHorizontalOffset) {
				if (!navHasTransform) {
					$el.find('.owl-nav').css({
						transform:
							'translate3d(' + self.options.navHorizontalOffset + ', ' + self.options.navVerticalOffset + ', 0)',
					});
				} else {
					$el.find('.owl-nav').css({
						top: 'calc( 50% - ' + self.options.navVerticalOffset + ' )',
			...
			if (self.options.dotsVerticalOffset && self.options.dotsHorizontalOffset) {
				$el.find('.owl-dots').css({
					transform:
						'translate3d(' + self.options.dotsHorizontalOffset + ', ' + self.options.dotsVerticalOffset + ', 0)',
				});
			}
		...
		build: function () {
			...
			if ($window.width() > 767) {
				...
				if (self.options.transition) {
					$el.css({
						transition:
							'ease-out transform ' + self.options.transitionDuration + 'ms ' + self.options.transitionDelay + 'ms',
					});
				}
	...
	PluginGDPRWrapper.prototype = {
		...
		build: function () {
			...
			if (
				...
				$.cookie('porto-gdpr-preferences').indexOf(self.options.checkCookie) != -1
			) {
				$.ajax({
...
(function (theme, $) {
	...
	PluginIcon.prototype = {
		...
		startIconAnimation: function (icon, $el) {
			...
			$({ to: 0 }).animate(
				{ to: 1 },
				self.options.strokeBased ? self.options.delay : self.options.delay + 300,
				function () {
					$el.css({ opacity: 1 });
				}
			);
		...
		build: function () {
			...
			if (self.options.scrollableParallax && $(window).width() > self.options.scrollableParallaxMinWidth) {
				...
				if ($scrollableWrapper.get(0)) {
					var progress =
							$(window).scrollTop() > self.options.wrapper.offset().top + $(window).outerHeight()
					...
					$(window).on('scroll', function (e) {
						if (self.options.wrapper.visible(true)) {
							...
							var scrollPercent = Math.abs(
								+(currentElementOffset - $window.height()) / (self.options.startOffset ? self.options.startOffset : 7)
							);
...
(function (theme, $) {
	...
	PluginReadMore.prototype = {
		...
		build: function () {
			...
			if ($('html').hasClass('safari')) {
				backgroundCssValue =
					'-webkit-linear-gradient(top, rgba(2, 0, 36, 0) 0%, ' + self.options.overlayColor + ' 100%)';
			}
...
(function (theme, $) {
	...
	PluginScrollSpy.prototype = {
		...
		build: function () {
			var self = this,
				target =
					document.querySelector(self.options.target) != null ? document.querySelector(self.options.target) : false,
				navItems =
	...
	PluginSectionScroll.prototype = {
		...
		build: function () {
			...
			$(window).on(wheelEvent, function (e) {
				...
				var wheelDirection =
					e.originalEvent.wheelDelta == undefined ? e.originalEvent.deltaY > 0 : e.originalEvent.wheelDelta < 0;
				if ($(window).width() < 992 && $('html').hasClass('touch')) {
...
(function (theme, $) {
	...
	PluginSort.prototype = {
		...
		setFilter: function (filter) {
			...
			self.$source
				.find('li[data-option-value="' + filter + '"], li[data-option-value="' + filter + '"] > a')
				.addClass('active');
	...
	PluginSticky.prototype = {
		...
		events: function () {
			var self = this,
				...
				class_to_check = self.options.wrapper.hasClass('sticky-wrapper-effect-1')
				...
				if ($html.hasClass('side-header') || $html.hasClass('side-header-hamburguer-sidebar')) {
					...
					self.checkReverse = function () {
						if (!checkReverseFlag) {
							...
							self.$wrapper
								.find('.dropdown:not(.manual):not(.dropdown-mega), .dropdown-submenu:not(.manual)')
								.each(function () {
						...
						menuNav: function () {
							...
							self.$mainNavItem.on('click', function (e) {
								var currentMenuItem = $(this),
									...
									nextMenuHeightDiff =
										nextMenu.find('> li').length * nextMenu.find('> li').outerHeight() - nextMenu.outerHeight(),
									prevMenuHeightDiff =
		...
		StickyHeader: {
			...
			build: function () {
				...
				if (self.options.stickyHeaderContainerHeight) {
					self.options.wrapper
						.find('.header-container')
						.css('height', self.options.wrapper.find('.header-container').outerHeight());
				}
